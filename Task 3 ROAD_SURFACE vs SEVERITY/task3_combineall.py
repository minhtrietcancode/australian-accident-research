...
ax1.set_xlabel('Road Surface', fontsize=9)
ax1.set_ylabel('Value', fontsize=9)
ax1.set_title('Severity: Mean & Variance', fontsize=10)
ax1.set_xticks(index)
ax1.set_xticklabels(severity_stats['ROAD_SURFACE_CATEGORY'], rotation=45, ha='right', fontsize=7)
...
sorted_data = road_surface_serious_prob.sort_values('SERIOUS_AND_FATAL_ACCIDENT_PROB', ascending=False)

# Create bars on the second axis with reduced width
prob_bars = ax2.bar(sorted_data['ROAD_SURFACE_CATEGORY'], 
                   sorted_data['SERIOUS_AND_FATAL_ACCIDENT_PROB'],
                   width=0.4, color='#2ca02c', alpha=0.8)

# Add labels and title for the second subplot
ax2.set_xlabel('Road Surface', fontsize=9)
ax2.set_ylabel('Percentage (%)', fontsize=9)
ax2.set_title('Serious/Fatal Accidents Percentage', fontsize=10)
ax2.set_xticklabels(sorted_data['ROAD_SURFACE_CATEGORY'], rotation=45, ha='right', fontsize=7)
...
plt.savefig('surface_severity_final.png', dpi=300, bbox_inches='tight')
plt.close()

print("Combined visualization saved successfully as surface_severity_final.png!")